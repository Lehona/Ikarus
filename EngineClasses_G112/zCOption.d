...
class zCOption {
    var int _vbtl;                       //0x000
    var int m_bReadTemp;                 //0x004 //zBOOL //read "temporary" value instead of normal value.

    //zCArray<zCOptionSection*> sectionList; //an ini files consists of sections
        var int sectionList_array;       //0x008
        var int sectionList_numAlloc;    //0x00C
        var int sectionList_numInArray;  //0x010

    var int directory[NUM_ENTRIES];      //0x014 //zFILE*   //no idea what this is for.
    var string dir_string[NUM_ENTRIES];  //0x07C //zSTRING //no idea what this is for.
    var string commandline;              //0x284 //zSTRING //zCOptions is responsible for the command line as well.
};
